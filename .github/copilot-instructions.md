...
This is a single-page HTML5 Canvas game. All game logic is embedded in [gravityAssist.html](gravityAssist.html). Styling is in [css/style.css](css/style.css). There are no build steps, tests, or external dependencies — open `gravityAssist.html` in a browser to run.

## Big picture

- Single-page app: UI + game loop + entities all live in `gravityAssist.html`.
...
- Visual/style changes: edit [css/style.css](css/style.css) and the UI `#ui` markup at the top of `gravityAssist.html`.

## Project-specific conventions & patterns

- No modules: code is global and inline. Avoid renaming globals without updating all uses in `gravityAssist.html`.
...
- Run: open [gravityAssist.html](gravityAssist.html) in a desktop or mobile browser (no server required). For local file CORS issues, use a lightweight local server (e.g., `npx http-server .`) but typically not needed.
- Inspect: use browser DevTools console. Helpful places to add logs: `updatePlayer()`, `checkCollisions()`, and `checkLevel()`.
- Hot edits: editing `gravityAssist.html` and refreshing the page is the normal iteration loop.
...
- If extracting code into modules, do it in a new file and wire it in `<script>` tags — don't assume an existing bundler.
...
- Game entry & logic: [gravityAssist.html](gravityAssist.html)
